...
+
The above message means the certificate on the hub is not well set, please refer to xref:./requirements.adoc##openid-server-certificate[Securing OAuth requests to the OpenID Connect identity provider]
...
=== Invalid redirect URI: openid We Are Sorry... Client not found.

If you receive the following error:

image::./images/openid_client_not_found_error.png["We Are Sorry... Client not found"]

Verify the OpenID ClientID and redirect are correct. This most likely occurs if the OpenID setup was created incorrectly and there is a ClientID mismatch between what is defined in your AuthRealm `clientID` spec and what has been defined in OpenID itself.
